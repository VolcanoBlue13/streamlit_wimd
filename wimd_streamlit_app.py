...
def streamlit_wimd():
    ...
    with st.spinner('Updating Report...'):
        ...
        if major_grouping_column_data == "Population":
            ...
            column_data_to_plot = ['Number of children 0-5 (2020)', 
            ...
            'Total number of births (2020)', 'Population - all ages (2020)', 
            'Birth rate per 1000 (2020)', 'Number of births to mothers aged 24 and under (2019)', 
            'Number of births to mothers aged 25-34 (2019)', 
            'Number of births to mothers aged 35 and over (2019)', 
            'Total number of births (2019)', 'Population - all ages (2019)', 
            'Birth rate per 1000 (2019)', 'Number of births to mothers aged 24 and under (2018)', 
            'Number of births to mothers aged 25-34 (2018)', 
            'Number of births to mothers aged 35 and over (2018)', 
            'Total number of births (2018)', 'Population - all ages (2018)', 
            'Birth rate per 1000 (2018)', 'Number of births to mothers aged 24 and under (2017)', 
            'Number of births to mothers aged 25-34 (2017)', 'Number of births to mothers aged 35 and over (2017)',
            'Total number of births (2017)', 'Population - all ages (2017)', 'Birth rate per 1000 (2017)',
            'Change in birth rates from 2018 to 2020', 'Change in birth rates from 2017 to 2020']

        elif major_grouping_column_data == "WIMD":
            # Same here but with the WIMD data
            column_data_to_plot = ['WIMD decile', 'WIMD income deprivation count (0-4)', 
            'WIMD income domain deciles', 'WIMD income domain - percentage of people in income deprivation',
            'WIMD employment domain deciles', 'WIMD employment domain - percentage of people in employment deprivation', 
            'WIMD health domain deciles', 'WIMD health domain - GP recorded chronic condition rate (per 100)', 
            'WIMD health domain - limiting long-term illness rate (per 100)', 'WIMD health domain - premature death rate (per 100,000)',
            'WIMD health domain - GP recorded mental health condition rate (per 100)', 'WIMD health domain - cancer incidence rate (per 100,000)', 
            'WIMD health domain - low birth weight, live single births less than 2.5 kg (percentage)', 
            'WIMD health domain - percentage of children aged 4-5 who are obese', 'WIMD education domain deciles', 
            'WIMD education domain - foundation phase average point score', 'WIMD education domain - key stage 2 average point score',
            'WIMD education domain - key stage 4 average point score', 'WIMD education domain - repeat absenteeism percentage', 
            'WIMD educaiton domain - percentage of key stage 4 leavers entering higher education', 'WIMD education domain - percentage of adults aged 25-64 with no qualifications', 
            'WIMD access to services domain deciles', 'WIMD access to services domain - percentage unavailability of broadband at 30 mb/s',
            'WIMD access to services - average public return traveltime to a pharmacy (mins)', 
            'WIMD access to services - average public return traveltime to a food shop (mins)', 
            'WIMD access to services - average public return traveltime to a GP surgery (mins)',
            'WIMD access to services - average public return traveltime to a post office (mins)',
            'WIMD access to services - average public return traveltime to a primary school (mins)', 
            'WIMD access to services - average public return traveltime to a public library (mins)',
            'WIMD access to services - average public return traveltime to a sports facility (mins)', 
            'WIMD access to services - average public return traveltime to a secondary school (mins)',
            'WIMD access to services - average private return traveltime to a pharmacy (mins)',
            'WIMD access to services - average private return traveltime to a food shop (mins)',
            'WIMD access to services - average private return traveltime to a GP surgery (mins)', 
            'WIMD access to services - average private return traveltime to a post office (mins)',
            'WIMD access to services - average private return traveltime to a primary school (mins)',
            'WIMD access to services - average private return traveltime to a public library (mins)', 
            'WIMD access to services - average private return traveltime to a sports facility (mins)',
            'WIMD access to services - average private return traveltime to a secondary school (mins)',
            'WIMD access to services - average private return traveltime to a petrol station (mins)',
            'WIMD housing domain deciles', 'WIMD housing domain - percentage of people in overcrowded households',
            ...
            'WIMD physical environment domain - ambient green space score', 'WIMD community safety domain deciles', 
            ...
            column_data_to_plot =  ['Mean foundation phase score', 
            ...
            column_data_to_plot = ['Rural-Urban classification']
        ...
        if len(column_selection) !=0:
            ...
            performance_selections = st.multiselect("Choose LSOA FPP performance to filter down", performance_filter)
        
        #filtering down on wimd decile
        if len(column_selection) !=0:
            wimd_filter =sorted(wimd_data["wimd_decile"].unique())
            wimd_selections = st.multiselect("Choose WIMD decile to filter down", wimd_filter)


        # I used the "Economic Region/ Economic Action Plan Area (EAP) Name" column as an equivalent for regions.
        region_filter=wimd_data["Economic Region/ Economic Action Plan Area (EAP) Name"].unique()
        ...
        if len(region_selections) != 0:
            local_authority_filter = wimd_data[wimd_data["Economic Region/ Economic Action Plan Area (EAP) Name"].isin(region_selections)]["Local Authority (LA) Name"].unique()
        ...
        if len(region_selections) != 0:
            lsoas_to_plot = list(wimd_data[wimd_data["Economic Region/ Economic Action Plan Area (EAP) Name"].isin(region_selections)]["lsoa_code"])
            if len(local_authority_selections) != 0:
                lsoas_to_plot = list(wimd_data[wimd_data["Local Authority (LA) Name"].isin(local_authority_selections)]["lsoa_code"])
        else:
        ...
        # we want it to be :Q
        columns_nominal = ["rural_urban_classification", "change_birth_rates_18_to_20", "change_birth_rates_17_to_20", "fpp_performance"]
        ...
        if len(performance_selections) != 0:
            data_for_map_performance = data_for_map[data_for_map["fpp_performance"].isin(performance_selections)]
            if len(wimd_selections) != 0:
                data_for_map_wimd = data_for_map_performance[data_for_map_performance["wimd_decile"].isin(wimd_selections)]
                data_for_map_everything_else = data_for_map[~((data_for_map["fpp_performance"].isin(performance_selections)) & (data_for_map["wimd_decile"].isin(wimd_selections)))]
                data_for_map_everything_else[column_selection_actual_column_name] = -1
                data_for_map = pd.concat([data_for_map_performance, data_for_map_everything_else], axis = 0)
            else:
                data_for_map_everything_else_performance = data_for_map_performance[~data_for_map_performance["fpp_performance"].isin(performance_selections)]
                data_for_map_everything_else_performance[column_selection_actual_column_name] = -1
                data_for_map = pd.concat([data_for_map_performance, data_for_map_everything_else_performance], axis = 0)
        else:
            if len(wimd_selections) != 0:
                data_for_map_wimd = data_for_map[data_for_map["wimd_decile"].isin(wimd_selections)]
                data_for_map_everything_else_wimd = data_for_map[~data_for_map["wimd_decile"].isin(wimd_selections)]
                data_for_map_everything_else_wimd[column_selection_actual_column_name] = -1
                data_for_map = pd.concat([data_for_map_wimd, data_for_map_everything_else_wimd], axis = 0)
        ...
        map = alt.Chart(regions).mark_geoshape(stroke="white").transform_lookup(
            ...
            from_= alt.LookupData(data_for_map, 'lsoa_code', [column_selection_actual_column_name,"lsoa_name_1", 'Rural/ Urban Settlement Classification (RU) Name',"Local Authority (LA) Name", "Economic Region/ Economic Action Plan Area (EAP) Name", "wimd_decile"]),
        ...
        data_for_bar_chart = data_for_bar_chart.rename(columns={"Economic Region/ Economic Action Plan Area (EAP) Name":"Region"})
        if len(performance_selections) != 0:
            data_for_bar_chart = data_for_bar_chart[data_for_bar_chart["fpp_performance"].isin(performance_selections)]
        if len(wimd_selections) != 0:
            data_for_bar_chart = data_for_bar_chart[data_for_bar_chart["wimd_decile"].isin(wimd_selections)]
        ...
        data_for_bar_chart = data_for_bar_chart.sort_values(by = column_selection_actual_column_name, ascending=ascending_val)[:no_to_show]
        ...
        ).add_selection(click).properties(width=400, height=175)
        ...
        st.altair_chart(alt.hconcat(map,bar_charts).configure_view(strokeWidth=0).resolve_scale("independent").configure_axis(labelLimit=0, titleLimit=0).configure_legend(labelLimit=0, titleLimit=0), use_container_width=False)
    ...
    with st.expander("Dataframe"):
        ...
        dataframe_to_plot = wimd_data.rename(columns=dict((v,k) for k,v in utils.COLUMN_NAME_MAPPINGS.items()))
        dataframe_to_plot = dataframe_to_plot.rename(columns={"Economic Region/ Economic Action Plan Area (EAP) Name":"Region"})
        # This is an empty dataframe so we can add column selection choices to it.
        columns_for_df = []
        if "Population" in major_grouping_column_data_df:
            # This is me selecting certain columns which I feel come under the wider group of Population. These huge arrays could go in utils but thought it would be useful to see them here.
            columns_for_df.append(['Number of children 0-5 (2020)', 
            'Number of births to mothers aged 24 and under (2020)', 
            'Number of births to mothers aged 25-34 (2020)', 
            'Number of births to mothers aged 35 and over (2020)', 
            'Total number of births (2020)', 'Population - all ages (2020)', 
        ...
        if "WIMD" in major_grouping_column_data_df:
            # Same here but with the WIMD data
            columns_for_df.append(['WIMD income deprivation count (0-4)', 
            'WIMD income domain deciles', 'WIMD income domain - percentage of people in income deprivation',
            'WIMD employment domain deciles', 'WIMD employment domain - percentage of people in employment deprivation', 
            'WIMD health domain deciles', 'WIMD health domain - GP recorded chronic condition rate (per 100)', 
            'WIMD health domain - limiting long-term illness rate (per 100)', 'WIMD health domain - premature death rate (per 100,000)',
        ...
        if "FPP" in major_grouping_column_data_df:
            # And again with the FPP data
            columns_for_df.append(['Mean foundation phase score', 
            'Mean foundation phase score (rounded)',
            'Foundation phase (mean difference)', 
            'Foundation phase performance'])
            # And I thought this was useful but couldn't fit it into any other larger group
        if "Rural-Urban" in major_grouping_column_data_df:
            columns_for_df.append(['Rural-Urban classification'])
        ...
        if len(selections_for_dataframe) == 0: 
            selections_for_dataframe = ["LSOA name (1)", "LA name", "Region", "Rural-Urban classification", "WIMD decile", "Foundation phase performance"]
        else:
        ## This combines the columns which I think you'll always want plus any selections made.
            selections_for_dataframe = ["LSOA name (1)", "LA name", "Region", "Rural-Urban classification", "WIMD decile", "Foundation phase performance"] + selections_for_dataframe
        ...
        performance_selections_df = st.multiselect("Optional FPP filter", performance_filter_df)
        ...
        region_filter_df=wimd_data["Economic Region/ Economic Action Plan Area (EAP) Name"].unique()
        region_selections_df=st.multiselect("Optional region filter", region_filter_df)
        ...
        if len(performance_selections_df) != 0:
            dataframe_to_plot = dataframe_to_plot[dataframe_to_plot["Foundation phase performance"].isin(performance_selections_df)]
        if len(wimd_selections_df) != 0:
                dataframe_to_plot = dataframe_to_plot[dataframe_to_plot["WIMD decile"].isin(wimd_selections_df)]
        ...
        st.dataframe(dataframe_to_plot[selections_for_dataframe].style.apply(performance_colour, axis=1))
