...
def streamlit_wimd():
    ...
    with st.spinner('Updating Report...'):
        ...
        region_filter=wimd_data["economic_region_economic_action_plan_area_eap_code"].unique()
        ...
        if len(region_selections) != 0:
            local_authority_filter = wimd_data[wimd_data["economic_region_economic_action_plan_area_eap_code"].isin(region_selections)]["Local Authority (LA) Name"].unique()
        ...
        map = alt.Chart(regions).mark_geoshape(stroke="white").transform_lookup(
            ...
            from_= alt.LookupData(wimd_data, 'lsoa_code', [column_selection_actual_column_name,"lsoa_name_1", 'Rural/ Urban Settlement Classification (RU) Name',"Local Authority (LA) Name", "Economic Region/ Economic Action Plan Area (EAP) Name"]),
    ...
    with st.expander("Dataframe"):
        ...
        if "WIMD" in major_grouping_column_data_df:
            # Same here but with the WIMD data
            columns_for_df.append(['WIMD decile', 'WIMD income deprivation count (0-4)', 
            'WIMD income domain deciles', 'WIMD income domain - percentage of people in income deprivation',
            'WIMD employment domain deciles', 'WIMD employment domain - percentage of people in employment deprivation', 
            'WIMD health domain deciles', 'WIMD health domain - GP recorded chronic condition rate (per 100)', 
            'WIMD health domain - limiting long-term illness rate (per 100)', 'WIMD health domain - premature death rate (per 100,000)',
        ...
        if len(selections_for_dataframe) == 0: 
            selections_for_dataframe = ["LSOA name (1)", "LA name", "Rural-Urban classification", "Foundation phase performance"]
        else:
        ## This combines the columns which I think you'll always want plus any selections made.
            selections_for_dataframe = ["LSOA name (1)", "LA name", "Rural-Urban classification", "Foundation phase performance"] + selections_for_dataframe
        ...
        st.dataframe(dataframe_to_plot[selections_for_dataframe].style.apply(lambda x: ['background-color: mistyrose']*column_number_to_highlight if x["Foundation phase performance"] == "Underperforming" else (['background-color: honeydew']*column_number_to_highlight if x["Foundation phase performance"] == "Outperforming" else ['background-color: white']*column_number_to_highlight), axis=1))
